...
update_leaderboard_prizes <- function(spreadsheet_name = "(HS) leaderboards", game_folder = "homestreet") {
  ...
  clean_df <- spreadsheet_title %>%
    googlesheets::gs_title() %>%
    googlesheets::gs_read(ws = "guild_events") %>%
    .[-(1:3), -1]
  csv_location <- paste0(source_folder, "/csv/leagues.csv")
  ...
  df <- spreadsheet_title %>%
    googlesheets::gs_title() %>%
    googlesheets::gs_read(ws = "token_events")
  csv_location <- paste0(source_folder, "/csv/leagues.csv")
