update_balance_data_button <- function(input, output, session, name) {
  ...
  observeEvent(input[[button_name]], {
    ...
    if(name == 'economy'){
      ...
      update_functions[[name]](economy_file = input[['combobox.stories.economy_file']], game_folder = input[[game_folder_name]], release_version = input[['combobox.stories.economy_file']] %>% str_extract_all('[[0-9]]') %>% unlist() %>% paste(collapse = '') %>% as.numeric(), prod_checkbox = input[["checkboxInput.stories.prod"]], loc_checkbox = input[["checkboxInput.stories.localisation"]], scripts_checkbox = input[["checkboxInput.stories.scripts"]])
    }else{
